...
void rotarRoles(Jugador *jugadores, int numJugadores) {
    ...
    for (int i = 0; i < numJugadores; i++) {
        // Rotamos los roles
        if (jugadores[i].rol == 0) {
            jugadores[i].rol = 1;  // Small Blind
        } else if (jugadores[i].rol == 1) {
            jugadores[i].rol = 2;  // Big Blind
        } else if (jugadores[i].rol == 2) {
            jugadores[i].rol = 0;  // Ningún rol (después del Big Blind)
        }
    }
}
